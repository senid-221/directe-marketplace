...
export default async function ProductsPage({ searchParams }: { searchParams: Promise<{ sort?: string; min?: string; max?: string }> }) {
  ...
  return <main style={{maxWidth:1280,margin:"0 auto",padding:"30px 20px 60px"}}>
    ...
    <div className="products">{products.map((product)=><article className="card" key={product.id}><Link href={"/product/"+product.slug}><div className="cardImage">{product.images[0]?<img src={product.images[0].url} alt={product.images[0].alt||product.name} style={{width:"100%",height:"100%",objectFit:"cover"}}/>:"🛍️"}</div></Link><div className="cardBody"><Link href={"/product/"+product.slug} className="title">{product.name}</Link><div className="rating"><span className="material-symbols-outlined ratingIcon">star</span> {Number(product.rating).toFixed(1)} · {product.seller.storeName}</div><div className="price">RWF {Number(product.price).toLocaleString()}</div><div className="cardFooter"><Link href={"/product/"+product.slug} className="add" style={{textAlign:"center"}}>View product</Link></div></div></article>)}</div>
