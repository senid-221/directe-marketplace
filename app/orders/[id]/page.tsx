...
export default async function OrderPage({params}:{params:Promise<{id:string}>}){
 ...
 if(!order) notFound();
 return <main style={{maxWidth:900,margin:"0 auto",padding:"40px 20px"}}><div className="sectionHeader"><h1>Order confirmed</h1></div><div className="checkoutCard"><h2>Order #{order.id.slice(-8).toUpperCase()}</h2><p>Status: <strong>{order.status}</strong></p>{order.items.map(i=><div key={i.id} style={{display:"flex",justifyContent:"space-between",padding:"12px 0",borderBottom:"1px solid #eee"}}><span>{i.product.name} × {i.quantity}</span><strong>RWF {(Number(i.unitPrice)*i.quantity).toLocaleString()}</strong></div>)}<div className="grand" style={{marginTop:18}}><span>Total</span><strong>RWF {Number(order.total).toLocaleString()}</strong></div><Link href="/products" className="cta" style={{display:"inline-block",marginTop:20}}>Continue shopping</Link></div></main>;
}