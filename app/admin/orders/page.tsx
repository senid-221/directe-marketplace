...
export default async function AdminOrdersPage(){
 ...
 return <main className="portal"><AdminSidebar active="Orders"/><section className="portalMain"><div className="portalTop"><div><div className="eyebrow">ADMIN PORTAL</div><h1>Orders</h1><p className="portalSub">Manage customer orders across the marketplace.</p></div></div><AdminOrdersClient initial={orders.map((o: (typeof orders)[number])=>({id:o.id,total:Number(o.total),status:o.status,createdAt:o.createdAt.toISOString(),user:o.user,items:o.items.map((i: (typeof o.items)[number])=>({id:i.id,quantity:i.quantity,unitPrice:Number(i.unitPrice),product:i.product.name,seller:i.product.seller.storeName}))}))}/></section></main>;
}
