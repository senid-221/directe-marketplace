...
export default async function AdminSellersPage(){
 ...
 return <main className="portal"><AdminSidebar active="Sellers"/><section className="portalMain"><div className="portalTop"><div><div className="eyebrow">ADMIN PORTAL</div><h1>Sellers</h1><p className="portalSub">Manage seller applications and account status.</p></div></div><AdminSellersClient initial={sellers.map(s=>({id:s.id,storeName:s.storeName,status:s.status,createdAt:s.createdAt.toISOString(),description:s.description||"",productCount:s._count.products,user:s.user}))}/></section></main>;
}
