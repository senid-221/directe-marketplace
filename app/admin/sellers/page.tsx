...
export default async function AdminSellersPage(){
 ...
 return <main className="portal"><AdminSidebar active="Sellers"/><section className="portalMain"><div className="portalTop"><div><div className="eyebrow">ADMIN PORTAL</div><h1>Sellers</h1><p className="portalSub">Review paid seller applications and approve accounts.</p></div></div><AdminSellersClient initial={sellers.map((s: (typeof sellers)[number])=>({id:s.id,storeName:s.storeName,status:s.status,createdAt:s.createdAt.toISOString(),description:s.description||"",productCount:s._count.products,plan:s.plan,planAmount:s.planAmount?Number(s.planAmount):null,paymentStatus:s.paymentStatus,paymentMethod:s.paymentMethod,paymentTxRef:s.paymentTxRef,user:s.user}))}/></section></main>;
}
