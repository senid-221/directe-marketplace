...
export default async function AdminPage(){
 ...
 return <main className="portal"><AdminSidebar active="Overview"/><section className="portalMain">
  ...
  <div className="adminQuickLinks"><a href="/admin/sellers"><span className="material-symbols-outlined">storefront</span><strong>Seller approvals</strong><small>Review and approve sellers</small></a><a href="/admin/products"><span className="material-symbols-outlined">inventory_2</span><strong>Products</strong><small>Control marketplace listings</small></a><a href="/admin/orders"><span className="material-symbols-outlined">shopping_bag</span><strong>Orders</strong><small>Manage order status</small></a><a href="/admin/customers"><span className="material-symbols-outlined">group</span><strong>Customers</strong><small>View customer accounts</small></a><a href="/admin/payments"><span className="material-symbols-outlined">payments</span><strong>Payments</strong><small>Review transaction records</small></a><a href="/admin/categories"><span className="material-symbols-outlined">category</span><strong>Categories</strong><small>Manage catalog structure</small></a></div>
  <div className="panel"><div className="sectionHeader"><div><h2>Recent orders</h2><p className="portalSub">Latest marketplace activity.</p></div><a href="/admin/orders">View all</a></div><div className="sellerRecentOrders">{recentOrders.map(o=><div className="sellerRecentOrder" key={o.id}><div><strong>#{o.id.slice(-8).toUpperCase()}</strong><span>{o.user.name||o.user.phone||o.user.email||"Customer"}</span></div><div><span>{o.items.length} item{o.items.length===1?"":"s"}</span><strong>{o.status}</strong></div></div>)}{!recentOrders.length&&<div className="emptyState">No orders yet.</div>}</div></div>
 </section></main>;
}
