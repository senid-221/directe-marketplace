...
type CheckoutItem = { name: string; quantity: number; price: number; variantName: string | null };
...
export default async function CheckoutPage() {
  ...
  const items = await prisma.cartItem.findMany({ where: { userId: session.userId }, include: { product: true, variant: true } });
  const subtotal = items.reduce((sum:number,item:(typeof items)[number]) => sum + Number(item.variant?.price ?? item.product.price) * item.quantity, 0);
