...
export async function GET(){
  ...
  return NextResponse.json({products:products.map(p=>({id:p.id,name:p.name,price:Number(p.price),stock:p.stock,published:p.published,seller:p.seller.storeName,category:p.category.name,image:p.images[0]?.url||"",updatedAt:p.updatedAt}))});
}
