...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main style={{maxWidth:1280,margin:"0 auto",padding:"30px 20px"}}>
    ...
    <div className="productPage"><div>
    ...
    </div><section className="productInfo">
      <h1>{product.name}</h1><div className="productRating"><span className="material-symbols-outlined ratingIcon">star</span> {Number(product.rating).toFixed(1)} · {product.reviewCount} reviews</div>
      ...
      <ProductVariantPicker productId={product.id} variants={product.variants.map(v=>({id:v.id,name:v.name,price:v.price?Number(v.price):null,stock:v.stock}))}/>
